...
int convert_image_with_letterbox(image_buffer_t* src_image, image_buffer_t* dst_image, letterbox_t* letterbox, char color)
{
    ...
    if (dst_image->virt_addr == NULL && dst_image->fd <= 0) {
        ...
        set_image_dma_buf_alloc(dst_image);
...
int crop_and_resize_image(char* pSrc, int src_width, int src_height, image_buffer_t *img, int dst_width, int dst_height) {
    if ((img->virt_addr != NULL) && (img->width != dst_width || img->height != dst_height)) {
        free(img->virt_addr);
        memset(&img, 0, sizeof(image_buffer_t));
    }
    img->width = dst_width;
    img->height = dst_height;
    img->size = dst_width * dst_height * 3;
    img->format = IMAGE_FORMAT_RGB888;
    if (img->virt_addr == NULL) {
        img->virt_addr = (unsigned char*)malloc(img->size * sizeof(unsigned char));
    }
    ...
    rga_buffer_handle_t src_handle = importbuffer_virtualaddr(pSrc, src_buf_size);
    ...
    memcpy(img->virt_addr, dst_buf, dst_buf_size);
...
int test_rga_crop(char *pChar, int src_width, int src_height, char *pDst, int dst_width, int dst_height, int offset_width, int offset_height) {
    int ret = 0;
    ...
    src_buf_size = src_width * src_height * 3;
    dst_buf_size = dst_width * dst_height * 3;
    ...
    memcpy(pDst, dst_buf, dst_buf_size);
