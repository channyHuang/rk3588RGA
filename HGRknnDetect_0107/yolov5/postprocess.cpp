...
int OBJ_CLASS_NUM = 8;
float BOX_THRESH = 0.63f;
float BOX_THRESH_LIST[13] = {0.5, 0.5, 0.55, 0.5, 0.15, 0.5, 0.2, 0.15};
int anchor[13][6] = {{10, 13, 16, 30, 33, 23},
                          {30, 61, 62, 45, 59, 119},
                          {116, 90, 156, 198, 373, 326}};
int g_nOutputBranchNum = 3;
std::string g_sRootDirectory = "./";
std::string g_sLabelFileName = "labels_list.txt";
std::string g_sAnchorFileName = "anchors_yolov5.txt";
...
static int loadAnchor(const char *locationFilename) {
    printf("load anchor %s\n", locationFilename);
    ...
    while ((s = readLine(file, s, &n)) != NULL) {
        try {
            if (s == nullptr) break;
            int anchori = std::atoi(s);
            anchor[branch][i++] = anchori;
            if (i >= 6) {
                branch++;
                i = 0;
            }
            if (branch >= g_nOutputBranchNum) {
                break;
            }
        } catch (...) {
            printf("anchor file contains non int string(s)\n");
        } 
    }
    fclose(file);
    return 0;
...
static int process_i8(int8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
                      std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold, const float* threshold_list,
                      int32_t zp, float scale)
{
    int validCount = 0;
    int grid_len = grid_h * grid_w;
    int8_t thres_i8 = qnt_f32_to_affine(threshold, zp, scale);
    int8_t thres_i8_list[13] = {0};
    for (int i = 0; i < 13; ++i) {
        thres_i8_list[i] = qnt_f32_to_affine(threshold_list[i], zp, scale);
    }
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                ...
                if (box_confidence >= thres_i8)
                {
                    ...
                    if (maxClassProbs > thres_i8_list[maxClassId])
                    {
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, const float* conf_threshold_list, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < g_nOutputBranchNum; i++)
    {
        ...
        if (app_ctx->is_quant)
        {
            validCount += process_i8((int8_t *)_outputs[i].buf, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
                                     classId, conf_threshold, conf_threshold_list, 
...
{
    std::string LABEL_NALE_TXT_PATH = g_sRootDirectory + g_sLabelFileName;
    int ret = 0;
    ret = loadLabelName(LABEL_NALE_TXT_PATH.c_str(), labels);
    if (ret < 0)
    {
        printf("Load %s failed!\n", LABEL_NALE_TXT_PATH.c_str());
        return -1;
    }
    ret = loadAnchor(g_sAnchorFileName.c_str());
    if (ret < 0) {
        printf("Load anchor %s failed!\n", g_sAnchorFileName.c_str());
        return -1;
    }
    return 0;
}
