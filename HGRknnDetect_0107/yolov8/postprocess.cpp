...
// labels[OBJ_CLASS_NUM] valid
static char *labels[80];
int OBJ_CLASS_NUM = 13;
...
static int process_i8(int8_t *box_tensor, int32_t box_zp, float box_scale,
                      ...
                      std::vector<float> &objProbs, 
                      std::vector<int> &classId, 
                      float threshold, const float* threshold_list)
{
    int validCount = 0;
    int grid_len = grid_h * grid_w;
    ...
    int8_t score_thres_i8_list[13] = {0};
    for (int i = 0; i < 13; ++i) {
        score_thres_i8_list[i] = qnt_f32_to_affine(threshold_list[i], score_zp, score_scale);
    }
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            for (int c= 0; c< OBJ_CLASS_NUM; c++){
                if ((score_tensor[offset] > score_thres_i8_list[c]) && (score_tensor[offset] > max_score))
                {
                    max_score = score_tensor[offset];
                    max_class_id = c;
                }
            ...
            // compute box
            if (max_score> score_thres_i8_list[max_class_id]){
                offset = i* grid_w + j;
                float box[4];
                float before_dfl[dfl_len*4];
                for (int k=0; k< dfl_len*4; k++){
...
#endif
    int output_per_branch = app_ctx->io_num.n_output / g_nOutputBranchNum;
